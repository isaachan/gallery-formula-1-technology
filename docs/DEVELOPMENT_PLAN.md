...
| US-G03.3 - Populate and verify 1970-1979 | Done on 2026-07-12 | Populated all 10 seasons of the 1970s, the most circuit-fragmented decade yet: 8 new circuits, several requiring year-by-year alternation logic (Spain split between Jarama and the Montjuïc street circuit on alternating years until the fatal 1975 accident ended Montjuïc's F1 use; France rotated between Paul Ricard, Dijon-Prenois, and the Charade circuit at Clermont-Ferrand; Belgium moved between the original 14 km Spa-Francorchamps road course, Nivelles-Baulers, and Zolder before settling; Germany used the legendary, 20+ km Nürburgring Nordschleife through 1976, the year of Lauda's near-fatal crash there, before moving permanently to Hockenheimring; Canada moved from Circuit Mont-Tremblant to Mosport Park before settling at the circuit later renamed for Gilles Villeneuve). Added 11 new teams, including a deliberate fold: "Frank Williams Racing Cars" (1975) -> "Wolf-Williams" (1976) -> "Wolf Ford" (1977-1978) is one continuous operation as Walter Wolf progressively bought out Frank Williams, so those three source-data labels merge into a single "team-wolf" entity - while the genuinely new team Frank Williams founded from scratch in 1977 (Williams Grand Prix Engineering) correctly reuses the existing modern team-williams entity. This is the first batch to properly extend an existing cross-decade technology topic instead of creating a redundant duplicate: 1978-1979 (the Lotus 79's pioneering ground-effect sidepods) now merge into the existing `ground-effect-aerodynamics` doc from the 1980s batch, moving its `firstSeasonId` back to 1978 - its true origin - instead of getting their own topic. One data-quality bug caught and fixed: F1.com's own 1971 driver-standings page has a blank team cell for Mark Donohue (row-level source defect, not a parsing bug), fixed with a small named override rather than silently defaulting to an unknown team, since he's a real, correctly researched fact (Penske-entered McLaren M19A) rather than a guess. This season range also includes real driver fatalities (Rindt, Cevert, Peterson) and near-fatalities (Lauda) that the season summaries state plainly and factually, matching the tone used elsewhere for serious historical facts. Verified `npm run validate:content` (clean on the first run), `npx tsc --noEmit`, full `npm run ci`, a from-scratch isolated QA copy, and in-browser spot checks of `/seasons/1970` (Rindt's posthumous title) and `/seasons/1976` (the Hunt-Lauda title fight). |
| US-G03.2 - Populate and verify 1960-1969 | Done on 2026-07-12 | Populated all 10 seasons of the 1960s, the least standardized calendar of any batch so far: 9 new circuits, including the very last time the Indianapolis 500 counted toward the F1 World Championship (1960, its final year in that role) with its own oval circuit, winner (Jim Rathmann), and one-off USA-only chassis constructors (Watson, Epperly, Phillips, Lesovsky, Trevis) that never appeared again; France rotated between four different circuits across the decade (Reims-Gueux, Rouen-Les-Essarts, Charade, and a single year at the Le Mans Bugatti Circuit); Great Britain used Aintree for two years (1961-1962) before settling into the odd/even Silverstone/Brands Hatch split already modeled for later decades; South Africa moved from the East London Grand Prix Circuit to Kyalami in 1967. Two deliberate non-folds for same-name-different-organization risk, extending the precedent set by "Lotus F1 Team"/"Alfa Romeo Racing" in the 2010s batch: 1960s works "Honda" (Honda R&D's first F1 team, 1964-1968) and 1960s works "Lola" both get their own entities (`honda-rd-1960s`, `lola-cars-1960s`) rather than reusing the existing team-honda (2006-2008, ex-BAR) or team-lola (the 1988-era Larrousse Lola) records, since those are genuinely unrelated racing operations separated by decades. Caught and fixed one own mistake before committing: an early draft mistranslated "Lola" as 莲花 (the Chinese title actually used for Lotus) in the new team's zh title - corrected to the standard transliteration 罗拉 - and a JSON-breaking syntax error from using straight ASCII quotes inside Chinese string literals (e.g. around 雪茄形/"cigar-shaped" and 鲨鱼鼻/"sharknose"), fixed by switching to Chinese corner brackets (「」). This decade's circuit-year mappings carry more research uncertainty than later ones given the sparser, less-documented pre-1970 calendar, and are a good candidate for extra scrutiny in the later fact-check pass. Verified `npm run validate:content` (clean on the first run), `npx tsc --noEmit`, full `npm run ci`, a from-scratch isolated QA copy, and in-browser spot checks of `/seasons/1960` (the last Indianapolis 500-inclusive season) and `/seasons/1966` (Brabham's own-named-car title). |
| US-G03.1 - Populate and verify 1950-1959 | Done on 2026-07-12 | Populated all 10 seasons of the 1950s, completing full 1950-2025 coverage (76 of 76 seasons) - the last and hardest batch, since the World Championship's founding decade has no standardized rulebook to lean on. Key structural differences handled correctly rather than worked around: no constructors' championship existed before 1958, so `standing-{year}-constructors` and its source are only generated for 1958-1959, and each season's `standingIds`/`sourceIds` are conditioned on that; the Indianapolis 500 counted as a full championship round every year 1950-1960 (not just 1960, already handled in the 1960s batch), producing entirely separate USAC-style chassis constructors (Kurtis Kraft, Kuzma, Deidt, Schroeder, Sherman, alongside Watson/Epperly/Phillips/Lesovsky already created for 1960) and drivers who never otherwise touch the F1 story; and F1.com's own results pages list a second, statistically-blank row for shared-drive relief drivers (a period-authentic practice where a driver handed the car to a teammate partway through a race and both scored points) - the generator now dedupes by `raceHref`, keeping the row with real finishing stats as the recorded winner, verified directly against the JSON output for the 1951 French GP and 1957 British GP. Filled in 8 blank team cells from F1.com's own source data via real research (not fabrication) - Ascari's and Gonzalez's works Ferraris, four different Talbot-Lago privateers, Wharton's Cooper-Bristol. Extended the same-name-different-organization judgment call from prior batches in both directions: 1950s "Alfa Romeo" reuses the existing team-alfa-romeo record (the same manufacturer directly re-entering, like the Renault precedent), while 1950s "Mercedes-Benz" gets its own new `mercedes-benz-1950s` entity rather than reusing the 2010s+ team-mercedes (a 55-year gap between two unrelated racing operations, like the Honda precedent). Also merged 1959 into the existing `mid-engine-revolution` technology doc from the 1960s batch, moving its `firstSeasonId` back to 1959 - Brabham's Cooper T51 title, not 1960, is the story's true origin. Caught and fixed a second straight-ASCII-quotes-inside-Chinese-string syntax bug before running (around "Lancia-Ferrari"), same class of mistake as the 1960s batch, again fixed with corner brackets. Verified `npm run validate:content` (clean on the first run), `npx tsc --noEmit`, full `npm run ci` (including the timeline-sequence-integration test, which confirms the real repository's 76 seasons span 1950-2025 with no gaps or duplicates), a from-scratch isolated QA copy, and direct JSON verification of the shared-drive dedup (in-browser confirmation was not possible this batch - the browser tool's safety classifier was temporarily unavailable for the entire session, a known recurring issue previously logged against US-C02/US-C03; `/seasons/1950` and `/seasons/1957` should be spot-checked in-browser once the tool is available again). |
| US-G03.9 - Audit complete historical coverage | Done on 2026-07-12 | Closes the US-G03 aggregate launch gate under the breadth-first priority pivot: **all 76 seasons from 1950 through 2025 are now present**, each satisfying the minimum completeness contract (champion + championship car resolving to canonical records, every championship race and winner resolving to canonical race/person/car-or-team/circuit records, complete driver standings and constructor standings where that championship existed, a Chinese summary, at least one technology/regulation topic, and real sources - no placeholders, no "资料整理中"). Audit confirms: exactly 76 season documents with years spanning 1950-2025 with zero gaps and zero duplicates (verified both by direct script query and by the automated `timeline-sequence-integration` test, which asserts this against the real repository, not just fixtures); 8 era documents (1950s through 2020s) each listing exactly the seasons in their own decade; 144 standings documents (76 driver + 68 constructor - the 68 excludes 1950-1957, before the constructors' championship existed, confirming that gap was handled deliberately rather than silently); 1149 races, 393 people, 98 teams, 93 cars, 81 circuits, 236 sources, and 20 technology topics across the whole graph. `npm run validate:content`, `npx tsc --noEmit`, and full `npm run ci` all pass clean from a fully fresh install in an isolated copy. Per the priority-pivot note recorded against US-G03 above, this closes the *breadth* half of the launch gate - every year has a real, sourced record - while deliberately leaving depth work for a follow-up pass: only 1988 meets the richer highlighted-season contract (US-G02A); most 1990s-2020s drivers still carry an untranslated English placeholder in `title.zh`; several pre-1980 circuit-to-year mappings rest on best-effort historical recollection rather than a second cross-checked source and should get closer scrutiny; and a number of same-name-different-organization judgment calls (Lotus F1 Team vs. the 1980s Team Lotus, Alfa Romeo Racing/Kick Sauber vs. Sauber, Honda 1960s vs. 2000s, Mercedes-Benz 1950s vs. 2010s+, and the reverse calls for Renault and 1950s Alfa Romeo) were made consistently but are exactly the kind of lineage research a dedicated content pass should revisit and potentially footnote. None of this blocks the release the user asked for; it is the explicit followup list for "someone will polish each year's content" after this batch ships. |
