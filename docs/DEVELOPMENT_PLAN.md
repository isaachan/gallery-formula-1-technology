...
| US-C01 - Browse every season on the track | Partially done on 2026-07-11 | Completed child stories US-C01.1 through US-C01.7 (see below); **not yet done:** US-C01.8 (mobile performance verification) and full-scale rendering with real 1950–2025 content, since Epic G content authoring hasn't started. The home page demos the track with 76 placeholder-labeled seasons (clearly marked as demo data) rather than the `ContentRepository`'s `getTimeline()`, which should replace the demo data once real seasons exist. |
...
| US-C01.8 - Verify mobile timeline performance | Not started | No dedicated mobile-viewport (320–430px) performance measurement has been done yet; deferred until real content volume exists via Epic G, since demo-data performance would not represent the production case. |
