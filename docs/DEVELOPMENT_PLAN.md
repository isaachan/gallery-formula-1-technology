...
| US-G02 - Populate a season research packet | Partially done on 2026-07-11 | Added a "Season research packet checklist" to `content/README.md` covering every required field (champion/car, every race, standings with the Top-3-default rule, entrants, technology/regulation topics, Chinese editorial prose, sourcing, and explicit disclosure of anything scoped down or left unpopulated), written against and validated by the real US-G02A 1988 content. **Deferred, not yet done:** the "QA checks factual consistency against the agreed source hierarchy before the content batch is accepted" step is currently manual (cross-checking 2+ sources per consequential claim during authoring, as documented in the checklist) rather than an automated or independently-reviewed gate. |
| US-G02A - Validate the 1988 reference season | Partially done on 2026-07-11 | Populated real, cross-sourced 1988 season content into `content/`: era, season, all 16 championship races with real winners/dates/circuits, 16 real circuits, the championship-winning car (McLaren MP4/4, with real chassis/gearbox/suspension specifications from Honda's official site), 6 real drivers (the top 6 in the final standings) with real nationality/career-span/team data, 5 constructor teams, one technology (Honda RA168E) carrying all three required presentation types (a richText article, a real `animation` of the turbo cycle, and a real `model3d` of the engine — reusing existing original demo assets, not fabricated placeholders), and 11 real sources. Verified the primary journey (timeline deep link → season → car/person/technology subject pages → back to the correct timeline position) end-to-end in the browser against this real content, plus a live relationship-based museum search (searching a team name surfaces a driver linked to it only via `teamIds`). While authoring this, discovered and fixed a real, previously-unexercised architecture gap: the content schema requires media-bearing blocks to reference a `mediaId`/`entityIds` (so the schema can graph-validate them), but the block registry renderer expects a fully inline, resolved shape — nothing bridged the two because `content/` had always been empty until now. Added `ContentRepository.resolveBlocks()` to bridge every block type (image/gallery/diagram/animation/audio/video/model3d/relatedEntities), and fixed a second bug where `mediaAsset` documents (which have no `status`/lifecycle field per their own schema) were silently dropped by the default published-only filter. Both fixes are covered by new repository tests. **Deferred, not yet done, and explicitly scoped down from "complete" for this session:** driver standings cover the top 6 (not the full 17-driver grid) and constructor standings cover the top 5 (not the full 10-team grid) — every entry beyond that is deferred to the bulk US-G03 population, along with the entrant-car list (scoped to just the champion car, not all ~10 constructors' cars) and the two runner-up drivers' championship years from other, not-yet-populated seasons (recorded as empty rather than guessed, per the "record unknown/deferred information explicitly" rule, since the graph's own consistency check requires a championship year to resolve to a populated season). A mobile Lighthouse re-audit and a full accessibility pass specific to this real content were not run this session (the existing US-C01.8/US-H tooling was not re-invoked against real content). Verified `npm run ci` in the working tree and in an isolated QA copy under `/tmp/f1-qa-us-g02a.*`. |
