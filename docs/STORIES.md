...
| --- | --- | --- | --- | --- |
| US-UI-01 | 修复首页时间线底部空白区域 | 移动端访问首页时，时间线滚动区下方出现约 1/6-1/7 的空白。根因是 `.timeline-scroll` 的 `max-height: 70vh` 限制了滚动容器高度。采用 Flex 自动填充方案，让滚动区吃掉 topbar 之外的剩余视口空间。 | [docs/stories/US-UI-01-timeline-bottom-whitespace.md](stories/US-UI-01-timeline-bottom-whitespace.md) | backlog |
